...
namespace Canyon.Game.Sockets.Game.Packets
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<MsgSyndicate>();
        ...
        public override async Task ProcessAsync(Client client)
        {
            ...
            switch (Mode)
            {
                ...
                case SyndicateRequest.Bulletin:
                    {
                        ...
                        {
                            await user.SendAsync(string.Format(StrSynNoMoney, Syndicate.SYNDICATE_ACTION_COST));
                            return;
                        }
                ...
                case SyndicateRequest.SetRequirements:
                    {
                        ...
                        user.Syndicate.ProfessionRequirement = (Syndicate.ProfessionPermission)(uint)Math.Min((uint)Syndicate.ProfessionPermission.All, Math.Max(0, ConditionProfession));
                ...
                default:
                    logger.LogWarning($"Type: {Type}, Subtype: {Mode} not handled\n{PacketDump.Hex(Encode())}");
                    break;
            }
        }
    }
