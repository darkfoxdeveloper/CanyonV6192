...
{
    public sealed class MsgChangeName : MsgBase<Client>
    {
        private static readonly ILogger logger = LogFactory.CreateGmLogger("chg_name");
                ...
                case ChangeNameAction.Request:
                    {
                        ...
                        if (await CharacterRepository.ExistsAsync(Name))
                        {
                        ...
                        Name = Name.Replace(" ", "~");
                        await ServerDbContext.SaveAsync(new DbChangeNameBackup
                        {
                        ...
                        logger.LogInformation($"{user.Identity},{user.Name},{Name},{CHANGE_NAME_COST}");
                        ...
                        await user.SendAsync(this);
                        await BroadcastWorldMsgAsync(broadcastMessage, TalkChannel.Talk, Color.White);
                        await PeerageManager.ChangeNameAsync(user, Name);
                        break;
                    }
            }
        }
