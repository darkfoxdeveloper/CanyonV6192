...
namespace Canyon.Game.Sockets.Game.Packets
{
    public sealed class MsgTeam : MsgBase<Client>
    {
        ...
        private static readonly ILogger logger = LogFactory.CreateLogger<MsgTeam>();
        ...
        public override async Task ProcessAsync(Client client)
        {
            ...
            {
                logger.LogError("Team no target");
                return;
            }
            ...
            {
                logger.LogWarning($"GM Character trying to team with no GM");
                return;
            }
#endif

            if (user.Map.IsTeamDisable()) return;

            var currentEvent = user.GetCurrentEvent();
            if (currentEvent is TeamArenaQualifier teamQualifier && teamQualifier.FindMatch(user.Identity) != null)
            {
                // no team action in events
                return;
            }
            ...
            switch (Action)
            {
                case TeamAction.Create:
                    {
                        ...
                        Team team = TeamManager.Create(user);
                        if (team == null) return;
                        ...
                        {
                            await te.OnJoinTeamAsync(user, user);
                        }

                        await ProcessGoalManager.SetProgressAsync(user, ProcessGoalManager.GoalType.MakeJoinTeam, 1);
                        break;
                    }
                ...
                default:
                    {
                        logger.LogWarning($"MsgTeam:{Action} unhandled subtype");
                        break;
                    }
            }
        }
