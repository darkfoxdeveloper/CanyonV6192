...
{
    public sealed class MsgTotemsRegister : MsgBase<Client>
    {
        ...
        public override async Task ProcessAsync(Client client)
        {
            ...
            {
                Item item = user.UserPackage.FindByIdentity(idItem);
                if (item == null)
                {
                    continue;
                }
                ...
                if (!await user.Syndicate.InscribeItemAsync(user, item))
                {
                    continue;
                }
