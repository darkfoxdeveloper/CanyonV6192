...
namespace Canyon.Game.Sockets.Game.Packets
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<MsgFamilyOccupy>();
                ...
                case FamilyPromptType.RequestNpc:
                    {
                        ...
                        if (npc.OwnerIdentity == user.FamilyIdentity)
                        {
                            ...
                            SubAction = user.FamilyPosition == Family.FamilyRank.ClanLeader ? 1u : 2u;

                            CanClaimRevenue = user.FamilyPosition == Family.FamilyRank.ClanLeader && war.HasRewardToClaim(user);
