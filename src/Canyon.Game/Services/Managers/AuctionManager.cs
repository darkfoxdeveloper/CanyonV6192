...
using Canyon.Game.States.Auction;
...
namespace Canyon.Game.Services.Managers
{
    public class AuctionManager
    {
        ...
        private static readonly ILogger logger = LogFactory.CreateLogger<AuctionManager>();
        private static readonly ILogger gmLogger = LogFactory.CreateGmLogger("auction_manager");
        ...
        {
            logger.LogInformation($"Initializing Auction data");
        ...
        {
            Item item = sender.UserPackage[itemId];
            if (!IsItemAuctionable(item))
            {
                logger.LogWarning($"User[{sender.Identity},{sender.Name}] tried to auction invalid item {item.Identity} - {item.FullName}");
                return;
            }
            ...
            if (!await auctionItem.InitializeAsync())
            {
                logger.LogError("Error when initialize auction item!");
                return;
            }

            await ServerDbContext.SaveAsync(dbAuction);
            ...
            {
                await BroadcastWorldMsgAsync(string.Format(StrAuctionSellSpiritNotice, item.FullName), TalkChannel.Talk, Color.White);
            }

            await sender.SendAsync(new MsgItemInfo(item, MsgItemInfo.ItemMode.Auction));
            if (item.Quench != null)
            {
                await item.Quench.SendToAsync(sender);
            }
        ...
        public static async Task<bool> PlaceBidAsync(Character user, uint itemId, uint value, int amount = 1)
        {
            ...
            await BroadcastWorldMsgAsync(msg);
