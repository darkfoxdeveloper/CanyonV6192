...
using static Canyon.Game.States.Syndicates.SyndicateMember;

namespace Canyon.Game.Services.Managers
{
    public class AchievementManager
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<AchievementManager>();
        private static readonly ConcurrentDictionary<uint, DbAchievementType> achievements = new();

        public static async Task InitializeAsync()
        {
            logger.LogInformation("Initializing Achievement Manager");
        ...
        public static async Task<bool> ProcessAsync(Character user, int flag)
        {
            ...
            switch ((AchievementType)flag)
            {
                ...
                case AchievementType.Chasingthewind:
                    if (user.UserPackage[ItemPosition.Steed] == null)
                    {
                        return false;
                    }
                ...
                case AchievementType.Asfastaslightning:
                    if (user.UserPackage[ItemPosition.Steed]?.Plus != 12)
                    {
                        return false;
                    }
