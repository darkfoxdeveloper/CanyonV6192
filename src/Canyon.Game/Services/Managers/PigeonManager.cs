...
namespace Canyon.Game.Services.Managers
{
    public static class PigeonManager
    {
                                                 ...
                                                 bool forceShow = false)
        {
            ...
            if (!await sender.SpendConquerPointsAsync(PIGEON_PRICE, true, true))
            {
                if (showError)
                {
        ...
        public static async Task AdditionAsync(Character sender, MsgPigeon request)
        {
            ...
            switch (request.Mode)
            {
                case MsgPigeon.PigeonMode.Urgent:
                    if (!await sender.SpendConquerPointsAsync(PIGEON_ADDITION, true, true))
                    {
                        await sender.SendAsync(StrPigeonUrgentErrNoEmoney);
                        return;
                    }
                ...
                case MsgPigeon.PigeonMode.SuperUrgent:
                    if (!await sender.SpendConquerPointsAsync(PIGEON_TOP_ADDITION, true, true))
                    {
                        await sender.SendAsync(StrPigeonUrgentErrNoEmoney);
                        return;
                    }
        ...
        public static Task RebuildQueueAsync()
        {
            ...
            return ServerDbContext.SaveRangeAsync(nextPigeons.ToArray());
        }
        ...
        public static async Task SyncAsync()
        {
            ...
            await ServerDbContext.SaveAsync(currentPigeon = new DbPigeon
            {
            ...
            await RebuildQueueAsync();
            await BroadcastWorldMsgAsync(new MsgTalk(currentPigeon.UserIdentity, TalkChannel.Broadcast, Color.White,
        ...
        public static async Task SendToUserAsync(Character user)
        {
            ...
            {
                await user.SendAsync(new MsgTalk(currentPigeon.UserIdentity, TalkChannel.Broadcast, Color.White,
                                                 MsgTalk.ALLUSERS, currentPigeon.UserName, currentPigeon.Message));
            }
        }
