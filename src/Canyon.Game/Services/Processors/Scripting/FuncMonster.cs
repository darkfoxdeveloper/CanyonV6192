...
namespace Canyon.Game.Services.Processors.Scripting
{
    public sealed partial class LuaProcessor
    {
        ...
        public bool killMonsterDropItem(int userId, uint itemType)
        {
            ...
            else if (itemType == Item.TYPE_DRAGONBALL)
            {
                ...
                if (monster != null)
                {
                    if (user != null)
                    {
                        BroadcastWorldMsgAsync(string.Format(StrDragonBallDropped, user.Name, user.Map.Name)).GetAwaiter().GetResult();
        ...
        public bool killMonsterDropMoney(int userId, uint money)
        {
            ...
            monster.DropMoneyAsync(money, idUser, MapItem.DropMode.Common).GetAwaiter().GetResult();
            return true;
        }
        ...
        public bool CreateMonster(int ownerType, uint idOwner, uint idMap, int x, int y, uint idGenerator, uint idType, int data, string name)
        {
            ...
            if (monstertype == null)
            {
                logger.LogWarning($"CreateMonster invalid monstertype[{idType}]");
                return false;
            }
            ...
            if (map == null)
            {
                logger.LogWarning($"CreateMonster invalid map[{idMap}]");
                return false;
            }
