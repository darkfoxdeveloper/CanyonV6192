...
namespace Canyon.Game.Services.Processors.Scripting
{
    public sealed partial class LuaProcessor
    {
        private Item GetItem(int itemId)
        {
            ...
            {
                return user?.UserPackage[(uint)itemId];
            }
        }
        ...
        public long GetItemInt(int itemId, int index)
        {
            ...
            switch (index)
            {
                ...
                case G_ITEM_DelTime: return 0; // TODO change item del time mode
                case G_ITEM_SaveTime: return item.DeleteTime;
            ...
            int color)
        {
            ...
            if (it == null)
            {
                logger.LogWarning("[Script: {}] itemtype [{}] not found.", currentScript, itemType);
                return false;
            }
