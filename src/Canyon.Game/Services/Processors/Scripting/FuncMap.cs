...
namespace Canyon.Game.Services.Processors.Scripting
{
    public sealed partial class LuaProcessor
    {
        ...
        public bool DropMapItem(int mapId, int x, int y, int itemType)
        {
            ...
            if (mapItem.Create(gameMap, new Point(x, y), (uint)itemType, 0, 0, 0, 0, MapItem.DropMode.Common))
            {
                ...
                {
                    Kernel.Services.Processor.Queue(gameMap.Partition, mapItem.EnterMapAsync);
                }
            }
            else
            {
        ...
        public bool DropMultiItems(int mapId, int itemId, int initX, int initY, int endX, int endY, int count, int existTime)
        {
            ...
            if (itemtype == null)
            {
                logger.LogWarning($"DropMultiItems Invalid itemtype {itemId}");
                return false;
            }
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (!mapItem.Create(gameMap, pos, itemtype.Type, 0, 0, 0, 0, MapItem.DropMode.Common))
                {
                    IdentityManager.MapItem.ReturnIdentity(mapItem.Identity);
                    continue;
                }
        ...
        public bool MapUserExeFunc(int mapId, int data, string function)
        {
            ...
            foreach (var player in gameMap.QueryPlayers().Take(data))
            {
                player.QueueAction(() =>
                {
                    LuaScriptManager.Run(player, role, item, string.Empty, function);
