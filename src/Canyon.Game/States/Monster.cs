...
namespace Canyon.Game.States
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<Monster>();
        ...
        public static async Task<Monster> CreateCallPetAsync(Character caller, uint type, ushort x, ushort y)
        {
            ...
            if (!await pet.InitializeAsync(caller.MapIdentity, caller.X, caller.Y))
            {
        ...
        public override async Task EnterMapAsync()
        {
            ...
            if (IsCallPet())
            {
                await BroadcastNpcMsgAsync(new MsgAiRoleLogin(this));
            }
        }
        ...
        private async Task DropEquipmentAsync(uint owner)
        {
            ...
            if (rand < 60)
            {
                ...
                MapItemInfo info = await Item.CreateItemInfoAsync(monsterType, quality);
        ...
        public override async Task OnTimerAsync()
        {
            ...
            {
                logger.LogError(ex, $"Monster::OnTimerAsync() => {Identity}:{Name} Set ghost: {ex.Message}");
            }
            ...
            {
                logger.LogError(ex, $"Monster::OnTimerAsync() => {Identity}:{Name} LeaveMapAsync: {ex.Message}");
            }
            ...
            {
                logger.LogError(ex, $"Monster::OnTimerAsync() => {Identity}:{Name} Status: {ex.Message}");
            }
            ...
            {
                logger.LogError(ex, $"Monster::OnTimerAsync() => {Identity}:{Name} BattleSystem.ProcessAttackAsync(): {ex.Message}");
            }
            ...
            {
                logger.LogError(ex, $"Monster::OnTimerAsync() => {Identity}:{Name} MagicData.OnTimerAsync(): {ex.Message}");
            }
        }
