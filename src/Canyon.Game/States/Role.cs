...
using Canyon.World.Roles;
using System.Drawing;
using static Canyon.Game.Sockets.Game.Packets.MsgAction;
using static Canyon.Game.Sockets.Game.Packets.MsgDeadMark;
...
namespace Canyon.Game.States
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<Role>();
        ...
        {
            if (role.MapIdentity != MapIdentity && role.Map.BaseMapId != MapIdentity)
            {
                return int.MaxValue;
            }
        ...
        public async Task<bool> JumpPosAsync(int x, int y, bool sync = false)
        {
            ...
            if (IsPlayer())
            {
                ...
                if (EventManager.IsInEventMap(GameEvent.EventType.CaptureTheFlag, user)
                    ...
                    && Map.QueryRegion(Canyon.World.Enums.RegionType.FlagProtection, (ushort)x, (ushort)y))
                {
                    await user.KickbackAsync();
                    return false;
                }
        ...
        public async Task<bool> MoveTowardAsync(int direction, int mode, bool sync = false)
        {
            ...
            if (currentEvent is CaptureTheFlag
                    && QueryStatus(StatusSet.CTF_FLAG) != null
                    && Map.QueryRegion(Canyon.World.Enums.RegionType.FlagProtection, newX, newY))
            {
                await user.KickbackAsync();
                return false;
            }
            ...
            {
                await user.KickbackAsync();
                await user.SendAsync(StrDead, TalkChannel.TopLeft, Color.Red);
                return false;
            }
            ...
            if (vigor > 0)
            {
                await AddAttributesAsync(ClientUpdateType.Vigor, vigor * -1);
            }
        ...
        public virtual async Task ProcessOnMoveAsync()
        {
            ...
            await DetachStatusAsync(StatusSet.MAGIC_DEFENDER);
        ...
        {
            foreach (MapTrap trap in Map.Query9BlocksByPos(X, Y).Where(x => x is MapTrap).Cast<MapTrap>())
            {
        ...
        public virtual async Task ProcessAfterAttackAsync()
        {
            await DetachStatusAsync(StatusSet.INTENSIFY);
        }
        ...
        public virtual async Task<bool> AddAttributesAsync(ClientUpdateType type, long value)
        {
            ...
            switch (type)
            {
                ...
                default:
                    logger.LogWarning($"Role::AddAttributes {type} not handled");
                    return false;
            }
        ...
        public virtual async Task<bool> SetAttributesAsync(ClientUpdateType type, ulong value)
        {
            switch (type)
            {
                ...
                default:
                    logger.LogWarning($"Role::SetAttributes {type} not handled");
                    return false;
            }
        ...
        {
            logger.LogWarning($"{GetType().Name} - {Identity} has no SendAsync(IPacket) handler");
            return Task.CompletedTask;
        }

        public virtual Task SendAsync(byte[] msg)
        {
            logger.LogWarning($"{GetType().Name} - {Identity} has no SendAsync(byte[]) handler");
            return Task.CompletedTask;
        }

        public virtual Task SendSpawnToAsync(Character player)
        {
            logger.LogWarning($"{GetType().Name} - {Identity} has no SendSpawnToAsync handler");
            return Task.CompletedTask;
        }

        public virtual Task SendSpawnToAsync(Character player, int x, int y)
        {
            logger.LogWarning($"{GetType().Name} - {Identity} has no SendSpawnToAsync(player, x, y) handler");
            return Task.CompletedTask;
        }
        ...
        {
            return BroadcastRoomMsgAsync(new MsgTalk(Identity, channel, color ?? Color.White, message), true);
        }
        ...
        public const int MAX_STRENGTH_POINTS_VALUE = 40000; // Chi Points
