...
namespace Canyon.Game.States.World
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<GameMap>();
        ...
        public override async Task<bool> InitializeAsync()
        {
            ...
            foreach (DbPassway dbPassway in passways)
            {
                ...
                if (portal == null)
                {
                    logger.LogWarning("Could not find portal for passway [{Passway}]", dbPassway.Identity);
                    continue;
                }
        ...
        public async Task LoadTrapsAsync()
        {
            foreach (var dbTrap in (await TrapRepository.GetAsync()).Where(x => x.MapId == Identity))
            {
                ...
                {
                    logger.LogError($"Could not start system map trap for {Identity} > Trap {dbTrap.Id}");
                    continue;
                }
            }
        }
        ...
        public bool IsSuperPosition(int x, int y)
        {
            return GetBlock(GetBlockX(x), GetBlockY(y))?.RoleSet.Values
                                                       .Any(a => a.X == x && a.Y == y && a.IsAlive) != false;
        }
        ...
        public Task BroadcastMsgAsync(string message, TalkChannel channel = TalkChannel.TopLeft,
                                            Color? color = null)
        {
            return BroadcastMsgAsync(new MsgTalk(0, channel, color ?? Color.White, message));
        }
        ...
        public async Task BroadcastRoomMsgAsync(int x, int y, IPacket msg, uint exclude = 0)
        {
            ...
            foreach (Character user in users.Values)
            {
                ...
                if (user.Client != null)
                {
                    await user.Client.SendAsync(encoded);
                }
            }
        }
        ...
        public bool GetRebornMap(ref uint idMap, ref Point target)
        {
            ...
            if (targetMap == null)
            {
                logger.LogError($"Could not get reborn map [{Identity}]!");
                return false;
            }
...
#if DEBUG
            logger.LogDebug($"User has jumped into [map:{Identity}:{Name}] portal {idxPassage} ({source.X:000}, {source.Y:000})");
#endif
        ...
        {
            foreach (Character player in users.Values)
            {
        ...
        public async Task<bool> SaveAsync()
        {
            ...
            if (mapEntity != null)
            {
                return await ServerDbContext.SaveAsync(mapEntity);
            }

            return await ServerDbContext.SaveAsync(dynaMapEntity);
        }
        ...
        {
            if (dynaMapEntity != null)
                return BroadcastNpcMsgAsync(new MsgAiDynaMap(dynaMapEntity));
        ...
        {
            if (dynaMapEntity != null)
                return BroadcastNpcMsgAsync(new MsgAiDynaMap(Identity));
            return Task.CompletedTask;
        }
