...
namespace Canyon.Game.States
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<BattleSystem>();
        ...
        public static async Task<bool> IsTargetDodgedAsync(Role attacker, Role target)
        {
            ...
            int dodgeAdd = 0;
            if (target.IsPlayer() && attacker.MagicData.QueryMagic?.Sort != MagicData.MagicSort.Tripleattack)
            {
                dodgeAdd = 40;
            }
            ...
            else if (attacker.MagicData.QueryMagic?.Sort == MagicData.MagicSort.Tripleattack)
            {
                minHitRate = 40;
            }
