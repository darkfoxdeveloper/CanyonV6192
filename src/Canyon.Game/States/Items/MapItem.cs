...
namespace Canyon.Game.States.Items
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<MapItem>();
        ...
        public bool Create(GameMap map, Point pos, Item pInfo, uint idOwner, DropMode mode)
        {
        ...
        public bool CreateMoney(GameMap map, Point pos, uint dwMoney, uint idOwner, DropMode mode)
        {
        ...
        public async Task DisappearAsync()
        {
            if (itemInfo != null)
            {
                await itemInfo.DeleteAsync(Item.ChangeOwnerType.DeleteDroppedItem);
            }
        ...
        {
            if (!Bound || OwnerIdentity == player.Identity)
            {
