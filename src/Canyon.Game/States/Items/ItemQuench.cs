...
namespace Canyon.Game.States.Items
{
    public sealed class ItemQuench
    {
        ...
        private static readonly ILogger logger = LogFactory.CreateLogger<ItemQuench>();
        ...
        public async Task<QuenchData> AppendAsync(DbItemStatus status)
        {
            ...
            if (itemType == null)
            {
                logger.LogInformation("Invalid itemtype {ItemType} for ItemStatus {Id}", status.Status, status.Id);
                return null;
            }
            ...
            {
                logger.LogWarning("Status {Id} of type {Status} is not an artifact or refinery", status.Id, status.Status);
                return null;
            }

            if (data.HasExpired)
            {
                logger.LogInformation("Deleting expired artifact or refinery [{Id}]", data.ItemStatus.Id);
            ...
            {
                if (!ItemManager.QuenchInfoData((int)status.Data, out var quenchInfoData))
                {
                    if (data.ItemStatus.Id != 0)
                    {
                        await data.DeleteAsync();
                    }
                    logger.LogInformation("Invalid status data for refinery!!! {status}", status.Status);
                    return null;
                }
        ...
        public class QuenchData
        {
            ...
            public QuenchAttribute Attribute1 { get; set; }

            public uint Power2 => ItemStatus?.Power2 ?? 0;
            public QuenchAttribute Attribute2 { get; set; }
            ...
            {
                await ServerDbContext.SaveAsync(ItemStatus);
            }
        ...
        {
            public QuenchAttribute Attribute1 { get; init; }
            public QuenchAttribute Attribute2 { get; init; }
        }
        ...
        public enum QuenchAttribute : uint
        {
            None,
            MagicDefense = 1,
            CriticalStrike = 2,
            ...
            Detoxication = 7,
            Block = 8,
            Penetration = 9, // REF_PENES = 9, HEUEHUEHUE BRBRBR
            Intensification = 10,
            FireResist = 11,
            WaterResist = 12,
            WoodResist = 13,
