...
using static Canyon.Game.Sockets.Game.Packets.MsgWalk;
using static Canyon.Game.States.BattleSystem;
    ...
    public partial class MagicData
    {
            ...
            ushort y)
        {
            ...
            if (user != null && (user.Map.QueryRegion(RegionType.PkProtected, user.X, user.Y) 
                || user.Map.QueryRegion(RegionType.FlagProtection, user.X, user.Y)))
            {
                if (magic.Ground > 0)
                {
                    if (magic.Crime > 0)
            ...
            {
                if (currentEvent is ArenaQualifier arenaQualifier && arenaQualifier.IsInsideMatch(user.Identity))
                {
            ...
            if (this.role.IsWing && magic.Sort == MagicSort.Transform)
            {
                return (false, x, y);
            }
            ...
            if (this.role.GetDistance(x, y) > this.role.GetAttackRange(0) + magic.Distance + 1)
            {
                return (false, x, y);
            }
            ...
            {
                bool isBadStatus = IsBadlyStatus(magic.Status);
                                                        ...
                                                        AutoActive ucAutoActive = 0)
        {
            ...
            {
                logger.LogWarning($"invalid magic type: {usMagicType}, user[{role.Name}][{role.Identity}]");
                return false;
            }
        ...
        private async Task<bool> LaunchAsync(Magic magic, AutoActive autoActive)
        {
            ...
            try
            {
                ...
                switch (magic.Sort)
                {
                    ...
                    default:
                        logger.LogWarning($"MagicProcessing::LaunchAsync AutoActive[{autoActive}] {magic.Name} {magic.Sort} not handled!!!");
                        result = true;
                        break;
                }
            }
            ...
            {
                logger.LogCritical(ex, "Error on LaunchAsync(). {message}", ex.Message);
            }
        ...
        private async Task<bool> ProcessAttackAsync(Magic magic)
        {
            ...
            var result = await CalcPowerAsync(byMagic, role, targetRole, magicPower);
        ...
        private async Task<bool> ProcessFanAsync(Magic magic)
        {
            ...
            foreach (Role target in setTarget)
            {
                var result = await CalcPowerAsync(byMagic, role, target, magic.Power);
                int power = result.Damage;
        ...
        private async Task<bool> ProcessBombAsync(Magic magic)
        {
            ...
            foreach (Role target in result.Roles)
            {
                ...
                var atkResult = await CalcPowerAsync(HitByMagic(magic), role, target, magic.Power);
        ...
        private async Task<bool> ProcessAttachAsync(Magic magic)
        {
            ...
            if (power < 0)
            {
                logger.LogWarning($"Error magic type invalid power {magic.Type} {magic.Power}");
                return false;
            }
                ...
                case StatusSet.PATH_OF_SHADOW:
                    {
                        ...
                        else
                        {
                            ...
                            msg.Append(role.Identity, 1, false, InteractionEffect.None, (int)role.Identity, role.X, role.Y);
                        }
                        break;
                    }
        ...
        private async Task<bool> ProcessLineAsync(Magic magic)
        {
            ...
            foreach (Point point in setPoint)
            {
                ...
                var result = await CalcPowerAsync(HitByMagic(magic), role, target, magic.Power);
        ...
        private async Task<bool> ProcessAddManaAsync(Magic magic)
        {
            ...
            {
                logger.LogWarning($"Add mana unhandled target {magic.Target}");
                return false;
            }
        ...
        private async Task<bool> ProcessCallPetAsync(Magic magic)
        {
            ...
            //await user.CallPetAsync((uint)magic.Power, user.X, user.Y, (int)magic.StepSeconds);
        ...
        private async Task<bool> ProcessChainXpAsync(Magic magic)
        {
            ...
            if (idTarget == 0)
            {
                logger.LogWarning($"No target for active bolt skill");
                return false;
            }
            ...
            {
                var result = await CalcPowerAsync(byMagic, role, atkTarget, magic.Power);
        ...
        private async Task<bool> ProcessKnockbackAsync(Magic magic)
        {
            ...
            FacingDirection direction = (FacingDirection)Calculations.GetDirection(role.X, role.Y, target.X, target.Y);
            bool stun = false;
            ...
            var result = await CalcPowerAsync(hitByMagic, role, target, magic.Power);
            ...
            {
                result = new AttackResult(0, 0, InteractionEffect.None);
            }
        ...
        private async Task<bool> ProcessTeamFlagAsync(Magic magic)
        {
            ...
            if (magic.Power < 0)
            {
                logger.LogWarning($"ERROR: magic type {magic.Sort} status [{magic.Status}] invalid power");
                return false;
            }
        ...
        private async Task<bool> ProcessIncreaseBlockAsync(Magic magic)
        {
            ...
            if (power < 0)
            {
                logger.LogWarning($"Invalid power for {magic.Sort} [{power}]");
                return false;
            }
        ...
        private async Task<bool> ProcessOblivionAsync(Magic magic)
        {
            ...
            if (power < 0)
            {
                logger.LogWarning($"Invalid power for magic oblivion {power}");
                return false;
            }
        ...
        private async Task<bool> ProcessTripleAttackAsync(Magic magic)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (await IsTargetDodgedAsync(role, targetRole))
                {
                    attackResult = new AttackResult(0, 0, InteractionEffect.None);
                }
                else
                {
                    attackResult = await CalcPowerAsync(MagicType.None, role, targetRole, magic.Power);
                }
        ...
        private async Task<bool> ProcessCannonBarrageAsync(Magic magic)
        {
            ...
            foreach (Role target in result.Roles)
            {
                ...
                var atkResult = await CalcPowerAsync(HitByMagic(magic), role, target, power);
                int damage = atkResult.Damage;
                if (user?.IsLucky == true && await ChanceCalcAsync(1, 100))
                {
                    await user.SendEffectAsync("LuckyGuy", true);
        ...
        private async Task<bool> ProcessKinecticSparkActivationAsync(Magic magic)
        {
            ...
            {
                await role.AttachStatusAsync(StatusSet.KINETIC_SPARK, magic.Power, (int)magic.StepSeconds, (int)magic.ActiveTimes, magic);
                msg.Append(role.Identity, 1, true);
            }
            await role.BroadcastRoomMsgAsync(msg, true);
            return true;
        ...
        private async Task<bool> ProcessFatalSpinAsync(Magic magic)
        {
            ...
            foreach (var target in targets)
            {
                ...
                var result = await CalcPowerAsync(hitByMagic, role, target, power);
                int damage = result.Damage + result.ElementalDamage;
