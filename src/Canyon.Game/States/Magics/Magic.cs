...
using static Canyon.Game.Sockets.Game.Packets.MsgFlushExp;
using static Canyon.Game.States.Magics.MagicData;

namespace Canyon.Game.States.Magics
{
    public sealed class Magic
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<Magic>();
        ...
        public async Task<bool> CreateAsync(uint idMagicType, ushort level = 0)
        {
            ...
            if (magictype == null)
            {
                logger.LogError("Invalid magic type for [{type},{level}]", idMagicType, level);
                return false;
            }
        ...
        public bool Create(DbMagic magic)
        {
            ...
            if (magictype == null)
            {
                logger.LogError("Invalid magic type for [{id},{type},{level}]", magic.Id, magic.Type, magic.Level);
                return false;
            }
        ...
        public async Task<bool> ChangeLevelAsync(byte newLevel)
        {
            ...
            if (magicType == null)
            {
                logger.LogError($"Invalid new level for {magic.Type} > {newLevel}");
                return false;
            }
        ...
        {
            return ServerDbContext.SaveAsync(magic);
        }
