...
using static Canyon.Game.Services.Managers.AuctionManager;

namespace Canyon.Game.States.Auction
{
    public class AuctionItem
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<AuctionItem>();
        ...
        public async Task<bool> InitializeAsync()
        {
            if (User == null)
            {
                DbCharacter user = await CharacterRepository.FindByIdentityAsync(auction.Seller);
                if (user == null)
                {
                    logger.LogCritical("Could not create auction item for auction id [invalid seller]: {}", auction.Id);
                    return false;
                }
                ...
                {
                    logger.LogCritical("Could not create auction item for auction id [invalid item]: {}", auction.Id);
                    return false;
                }
                item = tempItem;
            ...
            else
            {
                Character user = User;
                SellerName = user.Name;
                item = user.UserPackage.FindByIdentity(auction.ItemId);
            }
            ...
            if (auction.Id != 0)
            {
                foreach (var dbAskBuy in await AuctionRepository.GetAskBuyAsync(auction.Id))
                {
                    ...
                    if (user == null)
                    {
                        logger.LogCritical("Could not create auction askbuy for auction id [invalid bidder({})]: {}", dbAskBuy.Buyer, auction.Id);
                        continue;
                    }
        ...
        public async Task CancelAsync()
        {
            ...
            await ServerDbContext.SaveRangeAsync(emails);
        }
        ...
        public async Task<bool> BuyoutAsync(Character buyer)
        {
            if (RemainingTime == AuctionRemainingTime.Expired)
            {
                logger.LogWarning("Attempt to buy expired auction!");
                return false;
            }
        ...
        public async Task TimeOutAsync()
        {
            if (RemainingTime != AuctionRemainingTime.Expired)
            {
                logger.LogWarning("Attempt of expire not expired auction!");
                return;
            }
        ...
        {
            return ServerDbContext.SaveAsync(auction);
        }
