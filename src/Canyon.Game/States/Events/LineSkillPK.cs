...
namespace Canyon.Game.States.Events
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<LineSkillPK>();
        private static readonly ILogger gmLog = LogFactory.CreateGmLogger("line_skill_pk");
        ...
        {
            Map = MapManager.GetMap(MAP_ID_I);
            if (Map == null)
            {
                logger.LogError($"Map {MAP_ID_I} not found!");
        ...
        public override bool IsAllowedToJoin(Role sender)
        {
            ...
            if (user.MagicData.Magics.Values.All(x => x.Sort != MagicData.MagicSort.Line))
            {
        ...
        public override bool IsAttackEnable(Role sender, Magic magic = null)
        {
            ...
            if (magic.Sort != MagicData.MagicSort.Line)
            {
                return false;
            }
            return true;
        ...
        {
            gmLog.LogInformation($"Entrance,{sender.Identity},{sender.Name},{sender.Client.IpAddress},{sender.Client.MacAddress}");
        ...
        {
            gmLog.LogInformation($"Exit    ,{sender.Identity},{sender.Name},{sender.Client.IpAddress},{sender.Client.MacAddress}");
        ...
        public override async Task OnTimerAsync()
        {
            ...
            if (IsEnded)
            {
                ...
                foreach (var participant in participants.Values)
                {
                    ...
                    if (user == null || user.MapIdentity != MAP_ID_I)
                    {
                        continue;
                    }
        ...
        private async Task RewardsAsync()
        {
            do
            {
                ...
                if (user == null)
                {
                    gmLog.LogInformation($"Reward Ignored due to user [{championInfo.Identity},{championInfo.Name}] offline!!!");
                ...
                {
                    gmLog.LogInformation($"Reward Ignored due to user [{championInfo.Identity},{championInfo.Name}] not enough space in inventory!!!");
                    participants.TryRemove(championInfo.Identity, out _);
                    continue;
                }
