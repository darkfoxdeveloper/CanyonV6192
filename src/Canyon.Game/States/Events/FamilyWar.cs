...
namespace Canyon.Game.States.Events
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<FamilyWar>();
        ...
        private ConcurrentDictionary<uint, DynamicNpc> familyWarNpcs = new();
        ...
        public override async Task OnTimerAsync()
        {
            ...
            if (stage == FamilyWarStage.Idle)
            {
                ...
                {
                    logger.LogInformation("Starting Clan Wars!");
            ...
            else if (stage == FamilyWarStage.Preparing)
            {
                if (time == 2030)
                {
                    foreach (var npc in familyWarNpcs.Values)
                    {
                        ...
                        Family owner = FamilyManager.GetFamily(npc.OwnerIdentity);
            ...
            else if (stage == FamilyWarStage.WaitingConfirmation)
            {
                ...
                {
                    foreach (Family family in FamilyManager.QueryFamilies(x => x.Challenge != 0))
                    {
                        family.Challenge = 0;
        ...
        public override async Task<bool> CreateAsync()
        {
            var npcs = RoleManager
                .QueryRoles(x => x is DynamicNpc dynaNpc && dynaNpc.Type == BaseNpc.ROLE_FAMILY_WAR_FLAG)
                .Cast<DynamicNpc>();
        ...
        public uint GetNextReward(uint idNpc)
        {
            if (familyWarNpcs.TryGetValue(idNpc, out var npc))
            {
                ...
                Family family = FamilyManager.GetFamily(npc.OwnerIdentity);
                if (family == null)
                {
        ...
        public uint GetNextWeekReward(uint idNpc)
        {
            ...
            int prizePoolId = npc.GetData(PrizePool);
            Family family = GetFamilyOwner(idNpc);
            if (family == null)
            {
                return prizePool[prizePoolId][0];
            }
        ...
        public Family GetFamilyOwner(uint idNpc)
        {
            ...
            return FamilyManager.GetOccupyOwner((uint)npc.Identity);
        }
        ...
        public List<Family> GetChallengersByNpc(uint idNpc)
        {
            return FamilyManager.QueryFamilies(x => x.Challenge == idNpc);
        }
        ...
        {
            Family family = FamilyManager.GetFamily(idFamily);
            if (family == null)
            {
                return 0;
            }
        ...
        public async Task<bool> ValidateResultAsync(Character user, uint idNpc)
        {
            ...
            else if (families.Count > 1)
            {
                ...
                Family current = FamilyManager.GetOccupyOwner((uint)npc.Identity);
                if (families.All(x => x.Key != current?.Identity))
                {
                    ...
                    Family winner = FamilyManager.GetFamily(bpDict.OrderByDescending(x => x.Value).FirstOrDefault().Key);
                    await SetWinnerAsync(npc, winner);
                    await npc.SaveAsync();
                }
                else
        ...
        private async Task SetWinnerAsync(DynamicNpc npc, Family family)
        {
            if (npc.OwnerIdentity != family.Identity)
            {
                // remove the old owner from the NPC!
                Family oldOwner = FamilyManager.GetFamily(npc.OwnerIdentity);
                if (oldOwner != null)
                {
                    oldOwner.Occupy = 0;
