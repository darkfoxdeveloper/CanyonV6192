...
namespace Canyon.Game.States.Events.Qualifier.TeamQualifier
{
    public sealed class TeamArenaQualifierMatch
    {
        ...
        private static readonly ILogger logger = LogFactory.CreateLogger<TeamArenaQualifierMatch>();
        ...
        public Team Team1 => Company1?.Team;
        ...
        public Team Team2 => Company2?.Team;
            ...
            TeamArenaQualifierCompany company2)
        {
            ...
            {
                logger.LogWarning($"Could not validate team! Team {company1.Identity} denied");
                return false;
            }

            if (!ValidateTeam(company2, qualifier))
            {
                logger.LogWarning($"Could not validate team2! Team {company2.Identity} denied");
                return false;
            }
            ...
            {
                Action = MsgQualifyingInteractive.InteractionType.Countdown,
        ...
        private async Task MoveToMapAsync(TeamArenaQualifierCompany company, TeamArenaQualifierCompany opponent)
        {
            ...
            foreach (Character sender in company.Participants.Values)
            {
                ...
                {
                    Action = MsgQualifyingInteractive.InteractionType.StartTheFight
                });
                ...
                await sender.SendAsync(new MsgTeamArenaInteractive
                {
                    Action = MsgQualifyingInteractive.InteractionType.Match,
                    Option = MsgQualifyingInteractive.QualifierDialogButton.MatchOn
                });
        ...
        public async Task<bool> StartAsync()
        {
            ...
            {
                logger.LogError("Could not initialize map for arena qualifier!!");
                return false;
            }
        ...
        public async Task VictoryAsync(Character user)
        {
            ...
            {
                Action = MsgQualifyingInteractive.InteractionType.Dialog,
                Option = MsgQualifyingInteractive.QualifierDialogButton.Win,
        ...
        public async Task DefeatAsync(Character user)
        {
            ...
            {
                Action = MsgQualifyingInteractive.InteractionType.Dialog,
                Option = MsgQualifyingInteractive.QualifierDialogButton.Lose,
                Identity = user.Identity,
                Name = user.Name,
                Rank = user.TeamQualifierRank,
                Points = (int)user.TeamQualifierPoints,
        ...
        public async Task<bool> FinishAsync(uint idWinner, uint idLoser)
        {
            ...
            await BroadcastWorldMsgAsync(string.Format(Language.StrTeamArenaMatchEnd, winner.Name, loser.Name),
        ...
        private async Task DestroyAsync(TeamArenaQualifierCompany company, bool notStarted = false)
        {
            foreach (Character participant in company.Participants.Values)
            {
                ...
                {
                    Action = MsgQualifyingInteractive.InteractionType.EndDialog,
                    Option = (MsgQualifyingInteractive.QualifierDialogButton)(Winner == participant.Team.TeamId ? 1 : 0),
        ...
        public async Task SendBoardAsync()
        {
            ...
            MsgArenicWitness msg = new MsgArenicWitness
            {
        ...
        {
            Kernel.Services.Processor.Queue(Map?.Partition ?? 0, task);
        }
    }
}