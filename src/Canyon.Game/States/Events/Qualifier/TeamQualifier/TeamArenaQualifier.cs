...
{
    public sealed class TeamArenaQualifier : GameEvent, ITeamEvent, IWitnessEvent
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<TeamArenaQualifier>();
        ...
        public override async Task<bool> CreateAsync()
        {
            ...
            if (Map == null)
            {
                logger.LogWarning($"Base Map {BASE_MAP_ID_U} not found Team Arena Qualifier");
                return false;
            }

            var dbArenics = await ArenicRepository.GetAsync(DateTime.Now.AddDays(-1), 1);
            ...
            dbArenics = await ArenicRepository.GetAsync(DateTime.Now, 1);
            foreach (var dbArenic in dbArenics)
            {
                ArenicInformation arenicInformation = new ArenicInformation(dbArenic);
                if (!await arenicInformation.InitializeAsync())
        ...
        public override async Task<bool> OnReviveAsync(Character sender, bool selfRevive)
        {
            ...
            await sender.SendAsync(new MsgArenicWitness
            {
        ...
        public override async Task OnBeKillAsync(Role attacker, Role target, Magic magic = null)
        {
            ...
            if (match == null)
            {
                logger.LogCritical("Target event kill not in event!!!! {} -> {}", attacker.Identity, target.Identity);
                return; // ????
            }
        ...
        private async Task UpdateArenicAsync(TeamArenaQualifierCompany match)
        {
            foreach (var player in match.Participants.Values)
            {
                    ...
                    || arenic.Date.Date != DateTime.Now.Date)
                {
                    ...
                    arenics.TryAdd(player.Identity, arenic = new ArenicInformation(player, 1));
                }
