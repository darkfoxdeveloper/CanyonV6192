...
using static Canyon.Game.Sockets.Game.Packets.MsgSelfSynMemAwardRank;

namespace Canyon.Game.States.Events
{
    public sealed class CaptureTheFlag : SyndicateGameEvent
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<CaptureTheFlag>();
        private static readonly ILogger flagLogger = LogFactory.CreateGmLogger("ctf_flag");
        private static readonly ILogger rewardLogger = LogFactory.CreateGmLogger("ctf_syn_reward");
        private static readonly ILogger meedHistoryLogger = LogFactory.CreateGmLogger("ctf_meed_history");
        private static readonly ILogger setMeedLogger = LogFactory.CreateGmLogger("ctf_set_meed");
        ...
        {
            logger.LogInformation("Loading Capture the flag required data");

            Map = MapManager.GetMap(CTF_MAP_ID);
            if (Map == null)
            {
                logger.LogError($"Could not find map {CTF_MAP_ID} for ctf");
                return false;
            }
            ...
            logger.LogInformation($"{SyndicateMeed.Count} meed set for event");
            ...
            logger.LogInformation($"{flags.Count} flags loaded at GM map");
            ...
            logger.LogInformation($"{poles.Count} CTF poles loaded with success");
        ...
        public override async Task SetMeedAsync(Character user, ulong money, uint emoney)
        {
            ...
            Syndicate syndicate = user.Syndicate;
            ...
            setMeedLogger.LogInformation($"{user.Identity},{user.Name},{syndicate.Identity},{syndicate.Name},{money},{setMeed.Money},{emoney},{setMeed.Emoney}");

            if (setMeed.Money > 0 && setMeed.Emoney > 0)
            {
                await BroadcastWorldMsgAsync(string.Format(StrSynSetMeedBoth, user.Name, syndicate.Name, setMeed.Money, setMeed.Emoney, Name), TalkChannel.System, Color.White);
            }
            else if (setMeed.Money > 0)
            {
                await BroadcastWorldMsgAsync(string.Format(StrSynSetMeedMoney, user.Name, syndicate.Name, setMeed.Money, Name), TalkChannel.System, Color.White);
            }
            else if (setMeed.Emoney > 0)
            {
                await BroadcastWorldMsgAsync(string.Format(StrSynSetMeedEmoney, user.Name, syndicate.Name, setMeed.Emoney, Name), TalkChannel.System, Color.White);
            }
        ...
        public override async Task OnBeKillAsync(Role attacker, Role target, Magic magic = null)
        {
            if (target is DynamicNpc dynamicNpc)
            {
                ...
                if (topScore != null)
                {
                    Syndicate syndicate = SyndicateManager.GetSyndicate(topScore.Identity);
        ...
        public override async Task OnTimerAsync()
        {
            ...
            {
                logger.LogInformation("Ending event!");
        ...
        public async Task DeliverSyndicateRewardAsync()
        {
            ...
            {
                Syndicate syndicate = SyndicateManager.GetSyndicate(score.SynId);
                ...
                if (idx == 1)
                {
                    await BroadcastWorldMsgAsync(string.Format(StrSynCompeteCtfChampion, score.SynName, money, emoney), TalkChannel.Talk, Color.White);
                    if (syndicate != null)
                    {
                ...
                {
                    await BroadcastWorldMsgAsync(string.Format(StrSynCompeteCtf2nd, score.SynName, money, emoney), TalkChannel.Talk, Color.White);
                    if (syndicate != null)
                    {
                ...
                {
                    await BroadcastWorldMsgAsync(string.Format(StrSynCompeteCtf3rd, score.SynName, money, emoney), TalkChannel.Talk, Color.White);
                    if (syndicate != null)
                    {
                ...
                {
                    await BroadcastWorldMsgAsync(string.Format(StrSynCompeteCtf4th, score.SynName, idx, money, emoney), TalkChannel.Talk, Color.White);
                    if (syndicate != null)
                    {
                ...
                if (syndicate == null)
                {
                    rewardLogger.LogInformation($"{idx},{score.SynId},{score.SynName},0,{money},{emoney}");
                    idx++;
                    continue;
                }
        ...
        public async Task DeliverMeedAsync()
        {
            ...
            foreach (var setMeed in SyndicateMeed.Values)
            {
                ...
                Syndicate syndicate = SyndicateManager.GetSyndicate(setMeed.SyndicateId);
                if (syndicate == null)
                {
                    meedHistoryLogger.LogInformation($"{setMeed.SyndicateId},{setMeed.SyndicateName},0,None,{synRank},0,{setMeed.Money},{setMeed.Emoney}");
                    continue;
                }

                if (!Scores.TryGetValue(setMeed.SyndicateId, out var score))
                {
                    SyndicateMember randomMember = await syndicate.QueryRandomMemberAsync();
                    ...
                    meedHistoryLogger.LogInformation($"{setMeed.SyndicateId},{setMeed.SyndicateName},{randomMember.UserIdentity},{randomMember.UserName},{synRank},0,{setMeed.Money},{setMeed.Emoney}");
                    continue;
                }
                ...
                {
                    SyndicateMember member = syndicate.QueryMember(userScore.UserId);
                    if (member != null)
                    {
                ...
                foreach (var userScore in validScores.OrderByDescending(x => x.Points))
                {
                    ...
                    meedHistoryLogger.LogInformation($"{setMeed.SyndicateId},{setMeed.SyndicateName},{userScore.UserId},{userScore.Name},{synRank},{rate * 100:0.000},{setMeed.Money},{setMeed.Emoney}");
            ...
            await ServerDbContext.SaveRangeAsync(mails);
            await ServerDbContext.SaveRangeAsync(MeedRecord.Values.Select(x => x.GetRecord()).ToList());
            ...
            foreach (var flag in flags.Values)
            {
                ...
                async Task repositionNpc()
                {
                    ...
                    if (await flag.Npc.ChangePosAsync(CTF_MAP_ID, (ushort)respawnPoint.Point.X, (ushort)respawnPoint.Point.Y))
                    {
                        ...
                        logger.LogDebug($"Flag spawned at {Map.Name} {flag.Npc.X},{flag.Npc.Y}");
                    }
                    else
                    {
                        flag.Deliver();
        ...
        private class SetMeedRank
        {
            ...
            public uint SyndicateId
            {
                ...
                set
                {
                    Syndicate syn = SyndicateManager.GetSyndicate(value);
                    if (syn == null)
                    {
                        setMeed.SynId = 0;
                        SyndicateName = "None";
            ...
            {
                return ServerDbContext.SaveAsync(setMeed);
            }
        ...
        private class MeedRecordRank
        {
            ...
            {
                DbCharacter user = await CharacterRepository.FindByIdentityAsync(PlayerId);
                if (user != null)
                {
        ...
        private class SynCompeteRank
        {
            ...
            public Task SaveAsync()
            {
                return ServerDbContext.SaveAsync(rank);
            }
        }
