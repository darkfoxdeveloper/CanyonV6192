...
namespace Canyon.Game.States.User
{
    public partial class Character
    {
        ...
        private static readonly ILogger dropMoneyLogger = LogFactory.CreateGmLogger("money_drop");
        private static readonly ILogger pickupMoneyLogger = LogFactory.CreateGmLogger("money_pickup");
        private static readonly ILogger dropItemLogger = LogFactory.CreateGmLogger("item_drop");
        private static readonly ILogger pickupItemLogger = LogFactory.CreateGmLogger("item_pickup");
        ...
        public async Task<bool> DropItemAsync(uint idItem, int x, int y)
        {
            ...
            dropItemLogger.LogInformation($"{Name}({Identity}) drop item:[id={item.Identity}, type={item.Type}], dur={item.Durability}, max_dur={item.OriginalMaximumDurability}\r\n\t{item.ToJson()}");
        ...
        public async Task<bool> DropSilverAsync(uint amount)
        {
            ...
            dropMoneyLogger.LogInformation($"{Identity},{Name},{amount}");
        ...
        public async Task<bool> PickMapItemAsync(uint idItem, bool priorizeSash = false)
        {
            ...
            if (mapItem.IsMoney())
            {
                ...
                pickupMoneyLogger.LogInformation($"{Identity},{Name},{mapItem.Money},{MapIdentity},{Map.Name},{X},{Y}");
            }
            else
            {
                ...
                {
                    logger.LogWarning("Attempt to get bound item from the floor!");
                    return false;
                }
                ...
                if (item != null)
                {
                    ...
                    pickupItemLogger.LogInformation($"{Identity},{Name},{mapItem.ItemIdentity},{mapItem.Itemtype},{MapIdentity},{Map.Name},{X},{Y}");
                }
            }
        ...
        private static readonly ILogger spendEquipItemLogger = LogFactory.CreateGmLogger("spend_item");
        ...
        public async Task SendDetainRewardAsync()
        {
            ...
            foreach (DbDetainedItem dbDetained in items)
            {
                ...
                else if (expired && notClaimed && item != null)
                {
                    ...
                    if (item.IsBound)
                    {
                        await item.DeleteAsync(Item.ChangeOwnerType.DetainEquipment);
                        await ServerDbContext.DeleteAsync(dbDetained);
                        continue;
                    }
        ...
        {
            get => character.ChestPackageSize;
            set => character.ChestPackageSize = value;
        }
        ...
        public async Task AddSashSpaceAsync(int count)
        {
            ...
            if (!await SpendConquerPointsAsync(price, true, true))
            {
                return;
            }
