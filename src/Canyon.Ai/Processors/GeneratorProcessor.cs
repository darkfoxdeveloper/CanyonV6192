...
namespace Canyon.Ai.Processors
{
    public class GeneratorProcessor : BackgroundService
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<GeneratorProcessor>();
        ...
        protected virtual async Task ProcessAsync(int partition)
        {
            while (!cancelReads.IsCancellationRequested)
            {
                ...
                {
                    logger.LogError(ex, "Error on process generator. {}", ex.Message);
                }
                finally
                {
