...
namespace Canyon.Network.Packets
{
    ...
    public sealed class PacketReader : BinaryReader, IDisposable
    {
        ...
        public PacketReader(byte[] bytes) : base(new MemoryStream(bytes), CodePagesEncodingProvider.Instance.GetEncoding(1252))
        {
        }
        ...
        {
            return CodePagesEncodingProvider.Instance.GetEncoding(1252).GetString(ReadBytes(fixedLength)).TrimEnd('\0');
        }
